...
type
  ...
  private
    FCommonRessourceLoaded: boolean;
...
begin
 ...
 PostMessage( Form_Principale.Handle, LM_MESSAGE_PRINCIPAL, mpSceneStartLoop, 0 );
 PostMessage( Form_Principale.Handle, LM_MESSAGE_PRINCIPAL, mpShowHomeScreen, 0 );
end;
...
procedure TForm_Principale.MyMessageHandler(var Message: TLMessage);
begin
 case Message.wParam of
  mpSceneStartLoop : begin
                       FScene.Start;
                       Window_Scene.Close;
                       FreeCommonRessource;
  end;
  mpShowHomeScreen : begin
                      if not FCommonRessourceLoaded then LoadCommonRessource;
                      FScene.LaunchStage( ScreenMap );
  end;
 end;//case
end;

procedure TForm_Principale.LoadCommonRessource;
var ima: TBGRABitmap;
begin
 FCommonRessourceLoaded := TRUE;
 ...
 FTileEngine.MapHoleColor.Value := ColorToBGRA( ColorButton1.ButtonColor, SE6.Value );
...
procedure TForm_Principale.FreeCommonRessource;
begin
 FreeAndNil( TileSetManager );

 FreeAndNil( FTileEngine );

 FreeAndNil( ScreenMap );

 FreeAndNil( FImageBackGround );

 FCommonRessourceLoaded := FALSE;
end;
